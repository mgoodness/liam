...
// Banner): the block-letter wordmark (wordmark.go) beside three lines of
...
// bannerWordmarkGap is the column gap between the wordmark and the identity
// text beside it.
...
func bannerLines(pal theme.Palette, version, providerName, model, cwd string) []line {
	wordmark := lipgloss.NewStyle().PaddingRight(bannerWordmarkGap).Render(renderWordmark())
