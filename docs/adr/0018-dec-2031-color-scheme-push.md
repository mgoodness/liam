...
## Addendum: herdr's own passthrough of the push is unreliable (not a liam bug)

Reported after #203 landed: "the banner and status area change right away when the OS switches, but the transcript and a single line at the very bottom wait until focus" — inside herdr specifically. A Model-level test harness (constructing `tui.Model` directly, feeding it `uv.DarkColorSchemeEvent`/`uv.LightColorSchemeEvent`, and inspecting both the rendered `View().Content` and `View().BackgroundColor`) could not reproduce the symptom: `m.pal`, the transcript's own rendered text, and the OSC-11 background paint all updated on the very next render, with or without a concurrent focus-triggered re-query in flight.

Following ADR-0010's addendum's own precedent (compare bare terminal against the multiplexer before blaming either), the reporting user confirmed: liam behaves correctly and consistently in bare Ghostty; inside herdr, the push "sometimes auto-switches, but often the transcript & bottom line stays wrong indefinitely." Unlike this ADR's own Verification table entry above (herdr "✓ — confirmed by direct raw-escape-sequence testing"), that test only confirmed herdr can forward the push at all in a single instance — not that it does so reliably across repeated real OS-appearance toggles in practice. The actual defect is herdr's own passthrough of an *unsolicited* push: unlike an OSC-11 query/reply (always answered, on request), herdr appears to sometimes hold the push bytes back from the child pty until some other event flushes them through — and the event that reliably does is an actual terminal focus change, which is exactly why the affected areas recover the instant the user refocuses. That "recovery on focus" isn't liam's mode-2031 path working late; it's #103's independent, always-reliable focus-triggered OSC-11 re-query picking up the slack, exactly the graceful-degradation behavior this ADR's Considered Options section already designed for ("Terminals without mode 2031 support see no change at all"). No liam-side fix applies here; this is a herdr passthrough-reliability issue worth reporting to herdr's own maintainers, not this repo.

One real, independent liam-side gap *was* found and fixed during this diagnosis, unrelated to herdr: `applyColorSchemePush` didn't clear `Model.themeDetectPending`, so a push landing while an unrelated #103 re-query was still in flight would stay withheld (`View().BackgroundColor` left `nil`) until that older query's own reply arrived, even though the push already had a trustworthy answer. Fixed by folding `themeDetectPending = false` into the shared `applyPalette` helper, covered by `TestColorSchemePushClearsInFlightDetectPending` (internal/tui/tui_test.go).
